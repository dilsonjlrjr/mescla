...
#   ./wails/scripts/build-all.sh windows    # só Windows (amd64, cross-compile)
#   ./wails/scripts/build-all.sh linux      # só Linux (amd64, via Docker)
#
# Por que assim:
...
# Script vive em wails/scripts/ — sobe 2 níveis pra rodar da raiz do módulo Go
# (go.mod fica em project/, um nível acima de wails/), senão o build em
...
build_mac() {
  ...
  GOOS=darwin GOARCH=arm64 CGO_ENABLED=1 \
    go build -ldflags "$LDFLAGS" -o "$DIST/${APP}-darwin-arm64" ./wails

  say "macOS amd64…"
  GOOS=darwin GOARCH=amd64 CGO_ENABLED=1 \
    go build -ldflags "$LDFLAGS" -o "$DIST/${APP}-darwin-amd64" ./wails
  ...
  if [ -f wails/build/darwin/icons.icns ]; then
    cp wails/build/darwin/icons.icns "$app_dir/Resources/icons.icns"
  elif [ -f wails/build/appicon.png ]; then
...
build_windows() {
  ...
  if command -v wails3 >/dev/null 2>&1; then
    wails3 generate syso -arch amd64 \
      -icon wails/build/windows/icon.ico \
      -manifest wails/build/windows/wails.exe.manifest \
      -info wails/build/windows/info.json \
      -out wails/rsrc_windows_amd64.syso
  else
    warn "wails3 CLI ausente — exe sairá SEM ícone (go install github.com/wailsapp/wails/v3/cmd/wails3@latest)"
  fi
  GOOS=windows GOARCH=amd64 CGO_ENABLED=0 \
    go build -ldflags "$LDFLAGS -H windowsgui" -o "$DIST/${APP}-windows-amd64.exe" ./wails
  rm -f wails/rsrc_windows_amd64.syso
